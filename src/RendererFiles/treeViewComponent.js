...
/*
########################################################################
# Awkward explicit inlining of 'this.indexItemTo_ringBufferIndexItem': #
########################################################################
...
const TREEVIEWrenderKind_Cursor = 1;
const TREEVIEWrenderKind_Create = 2;
const TREEVIEWrenderKind_Batch = 3;
const TREEVIEWrenderKind_Scroll = 4;
const TREEVIEWrenderKind_SetItems = 5;
const TREEVIEWrenderKind_FullReset = 6;
const TREEVIEWrenderKind_Scroll_PullDataDrawResult = 7;
const TREEVIEWrenderKind_Resize = 8;
...
class TreeViewComponent {
    ...
    event_click(event_clientY, event_target) {
        ...
        // See comment "Awkward explicit inlining of 'this.indexItemTo_ringBufferIndexItem'" for more information.
...
const TreeViewNodeKind_None = 0;
const TreeViewNodeKind_isExpandable_isExpanded = 1;
const TreeViewNodeKind_isExpandable_NOTisExpanded = 2;
const TreeViewNodeKind_NOTisExpandable_isExpanded = 3;
const TreeViewNodeKind_NOTisExpandable_NOTisExpanded = 4;
...
//
    ...
    ensureCapacityForInsertion(index_abstract, count_abstract) {
        ...
        if (requiredCapacity_literal <= this.capacity_literal) {
            return;
        }

        // Calculate the new capacity by doubling until it fits
        let capacityNew_literal = this.capacity_literal || 1; // Prevent infinite loops if capacity is 0
        let capacityNew_abstract = this.capacity_abstract || 1; // Prevent infinite loops if capacity is 0
        while (capacityNew_literal < requiredCapacity_literal) {
            capacityNew_literal *= 2;
            capacityNew_abstract *= 2;
        }

        // Safety check against integer overflow / negative bounds
        if (capacityNew_literal < this.capacity_literal) {
        ...
        this.data_literal = dataNew_literal;
        this.capacity_literal = capacityNew_literal;
        this.capacity_abstract = capacityNew_abstract;
    }
